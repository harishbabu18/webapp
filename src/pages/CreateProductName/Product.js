...
function Product ({fetchDetails,username}) {
    const classes = useStyles();
    const offset = 0;
    const max = 10

    const url = 'inventory?offset='+offset+'&max='+max
    fetchDetails(url)
    ...
    //       if(!this.shouldComponentRender()) return <LinearProgress />
      ...
      <Table className={classes.table} aria-label="customized table">
        ...
        <TableBody>
            <renderProductRow product={username}/>
        {username.map(renderProductRow)}
        </TableBody>
    ...
    const mapStateToProps = state => (console.log(state),{
        username:state.products.items.inventory,
